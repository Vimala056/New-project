...
`)}const H="jobTrackerStatus",Q="jobTrackerStatusUpdates",G="Not Applied";function C(){try{const e=localStorage.getItem(H);return e?JSON.parse(e):{}}catch{return{}}}function z(e){return C()[e]??G}function K(e,t,o=null){const a=C();if(a[e]=t,localStorage.setItem(H,JSON.stringify(a)),t!==G&&(t==="Applied"||t==="Rejected"||t==="Selected")){const s=W();s.unshift({jobId:e,title:o?.title??"",company:o?.company??"",status:t,dateChanged:new Date().toISOString()}),localStorage.setItem(Q,JSON.stringify(s.slice(0,50)))}}function W(){try{const e=localStorage.getItem(Q);return e?JSON.parse(e):[]}catch{return[]}}function L(e={}){return W().map(o=>({...o,title:o.title||e[o.jobId]?.title,company:o.company||e[o.jobId]?.company}))}const $="jobTrackerTestChecklist",_=[{id:"t1",label:"Preferences persist after refresh",howTo:"Save preferences on Settings, refresh page, confirm fields are prefilled."},{id:"t2",label:"Match score calculates correctly",howTo:"Set role keywords and skills, check job cards show match badges."},{id:"t3",label:'"Show only matches" toggle works',howTo:"Enable toggle on Dashboard, confirm only jobs above threshold appear."},{id:"t4",label:"Save job persists after refresh",howTo:"Save a job on Dashboard, refresh, confirm it appears on Saved."},{id:"t5",label:"Apply opens in new tab",howTo:"Click Apply on any job card, confirm new tab opens."},{id:"t6",label:"Status update persists after refresh",howTo:"Change job status to Applied, refresh, confirm status remains."},{id:"t7",label:"Status filter works correctly",howTo:"Filter by Applied on Dashboard, confirm only Applied jobs show."},{id:"t8",label:"Digest generates top 10 by score",howTo:"Generate digest, confirm 10 jobs sorted by match score."},{id:"t9",label:"Digest persists for the day",howTo:"Generate digest, refresh page, confirm same digest loads."},{id:"t10",label:"No console errors on main pages",howTo:"Navigate Dashboard, Saved, Digest, Settings; check DevTools console."}];function V(){try{const e=localStorage.getItem($);return e?JSON.parse(e):{}}catch{return{}}}function ke(e,t){const o=V();o[e]=t,localStorage.setItem($,JSON.stringify(o))}function X(e){return!!V()[e]}function Z(){return _.filter(e=>X(e.id)).length}function E(){return Z()===_.length}function be(){localStorage.removeItem($)}const ee="jobTrackerProof",ve=/^https?:\/\/.+/i;function m(e){if(!e||typeof e!="string")return!1;const t=e.trim();if(!t)return!1;try{return new URL(t),ve.test(t)}catch{return!1}}function v(){try{const e=localStorage.getItem(ee);return e?JSON.parse(e):{lovableLink:"",githubLink:"",deployedLink:""}}catch{return{lovableLink:"",githubLink:"",deployedLink:""}}}function Se(e,t){const o=v();o[e]=(t||"").trim(),localStorage.setItem(ee,JSON.stringify(o))}function B(){const e=v();return m(e.lovableLink)&&m(e.githubLink)&&m(e.deployedLink)}const U="job-tracker-saved",te=["Not Applied","Applied","Rejected","Selected"],oe=Ae(u,"location"),ae=["Remote","Hybrid","Onsite"],se=["Fresher","0-1","1-3","3-5"];function S(){try{const e=localStorage.getItem(U);return e?JSON.parse(e):[]}catch{return[]}}function we(e){const t=S();t.includes(e)||(t.push(e),localStorage.setItem(U,JSON.stringify(t)))}function ne(e){const t=S().filter(o=>o!==e);localStorage.setItem(U,JSON.stringify(t))}function _e(e){return e===0?"Today":e===1?"1 day ago":`${e} days ago`}function Ae(e,t){return[...new Set(e.map(a=>a[t]).filter(Boolean))].sort()}function F(e){if(!e)return 0;const t=e.match(/[\d.]+/);return t?parseFloat(t[0]):0}function Le(e,t,o){let a=[...e];if(t.keyword){const n=t.keyword.toLowerCase();a=a.filter(r=>r.title.toLowerCase().includes(n)||r.company.toLowerCase().includes(n))}if(t.location&&(a=a.filter(n=>n.location===t.location)),t.mode&&(a=a.filter(n=>n.mode===t.mode)),t.experience&&(a=a.filter(n=>n.experience===t.experience)),t.source&&(a=a.filter(n=>n.source===t.source)),t.status){const n={};a.forEach(r=>{n[r.id]=z(r.id)}),a=a.filter(r=>n[r.id]===t.status)}if(t.onlyMatches&&o){const n=o.minMatchScore??40;a=a.filter(r=>(r.matchScore??0)>=n)}const s=t.sort||"Latest";return s==="Latest"?a.sort((n,r)=>n.postedDaysAgo-r.postedDaysAgo):s==="Oldest"?a.sort((n,r)=>r.postedDaysAgo-n.postedDaysAgo):s==="Match Score"?a.sort((n,r)=>(r.matchScore??0)-(n.matchScore??0)):s==="Salary"&&a.sort((n,r)=>F(r.salaryRange)-F(n.salaryRange)),a}function re(){return`
  ...
  `}function ie(e){const t=e||{},o=l(t.roleKeywords||""),a=l(t.skills||""),s=t.experienceLevel||"",n=t.minMatchScore??40,r=t.preferredLocations||[],c=t.preferredMode||[];return`
      ...
      <div class="app-settings">
        ...
        <div class="app-settings__field">
          ...
          <select multiple class="kodnest-input app-settings__locations" size="5">
            ${oe.map(i=>`<option value="${l(i)}" ${r.includes(i)?"selected":""}>${l(i)}</option>`).join("")}
          </select>
        ...
        <div class="app-settings__field">
          ...
          <select class="kodnest-input app-settings__experience">
            ...
            ${se.map(i=>`<option value="${i}" ${s===i?"selected":""}>${i}</option>`).join("")}
          </select>
        </div>
        <div class="app-settings__field">
  ...
  `}function je(e){return e>=80?"match-score--high":e>=60?"match-score--medium":e>=40?"match-score--neutral":"match-score--low"}function le(e,t,o){const a=o??0,s=je(a),n=o!==void 0?`<span class="job-card__match ${s}">${a}% match</span>`:"",r=z(e.id),c=ce(r);return`
    <article class="job-card kodnest-card" data-job-id="${e.id}">
      ...
      <p class="job-card__posted">${_e(e.postedDaysAgo)}</p>
      <div class="job-card__status">
        ...
        <select class="kodnest-input job-card__status-select ${c}" data-status-for="${e.id}">
          ${te.map(i=>`<option value="${l(i)}" ${r===i?"selected":""}>${i}</option>`).join("")}
        </select>
      </div>
  ...
  `}function ce(e){return e==="Applied"?"status--applied":e==="Rejected"?"status--rejected":e==="Selected"?"status--selected":"status--neutral"}function De(e,t){const o=t.locations,a=["LinkedIn","Naukri","Indeed"];return`
    <div class="filter-bar">
      ...
      <select class="kodnest-input filter-bar__select" data-filter="mode">
        ...
        ${ae.map(s=>`<option value="${s}" ${e.mode===s?"selected":""}>${s}</option>`).join("")}
      </select>
      <select class="kodnest-input filter-bar__select" data-filter="experience">
        <option value="">All experience</option>
        ${se.map(s=>`<option value="${s}" ${e.experience===s?"selected":""}>${s}</option>`).join("")}
      </select>
      ...
      <select class="kodnest-input filter-bar__select" data-filter="status">
        ...
        ${te.map(s=>`<option value="${s}" ${e.status===s?"selected":""}>${s}</option>`).join("")}
      </select>
  ...
  `}function de(e={}){const t=h(),o=S(),a={locations:oe},s=u.map(i=>({...i,matchScore:T(i,t)})),n=Le(s,{sort:"Latest",...e},t),r=!k(),c=n.length===0?"No roles match your criteria. Adjust filters or lower threshold.":null;return`
    <div class="app-page app-page--dashboard">
      ...
      <div class="job-grid" id="job-grid">
        ${n.length?n.map(i=>le(i,o.includes(i.id),i.matchScore)).join(""):`<div class="kodnest-empty"><p class="kodnest-empty__message">${c||"No jobs match your filters."}</p></div>`}
      </div>
    </div>
  `}function Ie(){const e=S(),t=u.filter(a=>e.includes(a.id)),o=h();return`
    <div class="app-page app-page--saved">
      <h1 class="app-page__heading">Saved</h1>
      ${t.length?`<div class="job-grid">${t.map(a=>le(a,!0,T(a,o))).join("")}</div>`:'<div class="kodnest-empty"><p class="kodnest-empty__message">Saved jobs will appear here once you start tracking.</p></div>'}
    </div>
  `}function Pe(e){return e?new Date(e).toLocaleDateString("en-IN",{weekday:"long",year:"numeric",month:"long",day:"numeric"}):""}function Re(){const e=k();let t=b();if(!e)return`
    ...
    `;const o=!t,a=t?.jobs??[],s=t&&a.length===0;if(o){const i=L(j());return`
      <div class="app-page app-page--digest">
        <h1 class="app-page__heading">Digest</h1>
    ...
    `}const n=Pe(t.date),r=L(j());return`
    <div class="app-page app-page--digest">
      <h1 class="app-page__heading">Digest</h1>
      ${D(r)}
      <div class="digest-card">
          ...
          ${a.map(i=>`
            <div class="digest-job">
              <div class="digest-job__main">
                ...
                <span class="digest-job__score match-score--${xe(i.matchScore??0)}">${i.matchScore??0}% match</span>
              </div>
  ...
  `}function xe(e){return e>=80?"high":e>=60?"medium":e>=40?"neutral":"low"}function j(){const e={};return u.forEach(t=>{e[t.id]=t}),e}function Te(e){return e?new Date(e).toLocaleDateString("en-IN",{day:"numeric",month:"short",year:"numeric"}):""}function D(e){return!e||e.length===0?"":`
      ...
      <ul class="digest-status-updates__list">
        ${e.slice(0,10).map(t=>`
          <li class="digest-status-updates__item">
            ...
            <span class="digest-status-updates__status status-badge status-badge--${ce(t.status)}">${l(t.status)}</span>
            <span class="digest-status-updates__date">${l(Te(t.dateChanged))}</span>
          </li>
        `).join("")}
      </ul>
    </div>
  `}const Ce=[{id:"s1",label:"Preferences configured",check:()=>k()},{id:"s2",label:"Match score active",check:()=>k()},{id:"s3",label:"Jobs displayed",check:()=>!0},{id:"s4",label:"Save job works",check:()=>S().length>0},{id:"s5",label:"Status tracking",check:()=>Object.values(C()).some(e=>e!=="Not Applied")},{id:"s6",label:"Digest generated",check:()=>!!b()},{id:"s7",label:"Test checklist complete",check:()=>E()},{id:"s8",label:"Artifacts submitted",check:()=>B()}];function M(){const e=E(),t=B();if(e&&t)return"Shipped";const o=v(),a=!!(o.lovableLink?.trim()||o.githubLink?.trim()||o.deployedLink?.trim());return e||t||a?"In Progress":"Not Started"}function $e(){return`
  ...
  `}function Be(){const e=Z(),t=_.length,o=e===t;return`
      ...
      <div class="test-checklist kodnest-card">
        ${_.map(a=>`
          <label class="test-item">
            <input type="checkbox" class="test-item__checkbox" data-test-id="${a.id}" ${X(a.id)?"checked":""}>
...
------------------------------------------`}function Me(){const e=M();if(!(e==="Shipped")){const a=[];E()||a.push("all 10 tests"),B()||a.push("all 3 artifact links");const s=a.length?`Complete ${a.join(" and ")} to unlock Ship.`:"Complete all 10 tests and provide all 3 artifact links to unlock Ship.";return`
  ...
  `;function a(){o.remove(),document.body.style.overflow=""}o.addEventListener("click",s=>{(s.target===o||s.target.closest(".modal__close"))&&a()}),document.addEventListener("keydown",function s(n){n.key==="Escape"&&(a(),document.removeEventListener("keydown",s))}),document.body.style.overflow="hidden",document.body.appendChild(o)}const Je={"/":re,"/dashboard":de,"/saved":Ie,"/digest":Re,"/settings":()=>ie(h()),"/proof":$e,"/jt/07-test":Be,"/jt/08-ship":Me,"/jt/proof":Ee};let w={};function N(){return window.location.pathname.replace(/\/$/,"")||"/"}function d(e){const t=document.getElementById("main-content");if(e==="/dashboard")t.innerHTML=de(w),I(t);else if(e==="/settings")t.innerHTML=ie(h()),qe(t);else{const o=Je[e]||re;t.innerHTML=o(),e==="/saved"&&R(t),e==="/digest"&&ue(t),e==="/jt/07-test"&&P(t),e==="/jt/proof"&&ge(t)}}function qe(e){const t=e.querySelector(".app-settings__save");if(!t)return;t.addEventListener("click",()=>{const s=e.querySelector(".app-settings__role-keywords")?.value??"",n=e.querySelector(".app-settings__skills")?.value??"",r=e.querySelector(".app-settings__experience")?.value??"",c=e.querySelector(".app-settings__slider"),i=c?parseInt(c.value,10):40,p=e.querySelector(".app-settings__locations"),f=p?[...p.selectedOptions].map(A=>A.value):[],g=[];e.querySelectorAll(".app-settings__checkbox input:checked").forEach(A=>{g.push(A.value)}),he({roleKeywords:s,preferredLocations:f,preferredMode:g,experienceLevel:r,skills:n,minMatchScore:i}),t.textContent="Saved",t.disabled=!0,setTimeout(()=>{t.textContent="Save Preferences",t.disabled=!1},1500)});const o=e.querySelector(".app-settings__slider"),a=e.querySelector("#min-match-label");o&&a&&o.addEventListener("input",s=>{a.textContent=`Min match score (${s.target.value})`})}function I(e){const t=Fe(o=>{w.keyword=o.trim()||void 0,d("/dashboard")},300);e.querySelectorAll(".filter-bar__search").forEach(o=>{o.addEventListener("input",a=>t(a.target.value))}),e.querySelectorAll(".filter-bar__select").forEach(o=>{o.addEventListener("change",a=>{const s=o.dataset.filter;w[s]=o.value||void 0,d("/dashboard")})}),e.querySelectorAll(".app-toggle__input").forEach(o=>{o.addEventListener("change",a=>{w.onlyMatches=a.target.checked,d("/dashboard")})}),e.querySelectorAll(".job-action[data-action='view']").forEach(o=>{o.addEventListener("click",()=>{const a=u.find(s=>s.id===o.dataset.id);a&&pe(a)})}),e.querySelectorAll(".job-action[data-action='save'], .job-action[data-action='unsave']").forEach(o=>{o.addEventListener("click",()=>{const a=o.dataset.id;o.dataset.action==="save"?we(a):ne(a),d("/dashboard"),I(document.getElementById("main-content"))})}),e.querySelectorAll(".job-card__status-select").forEach(o=>{o.addEventListener("change",a=>{const s=o.dataset.statusFor,n=a.target.value,r=u.find(c=>c.id===s);K(s,n,r),(n==="Applied"||n==="Rejected"||n==="Selected")&&y(`Status updated: ${n}`),d("/dashboard"),I(document.getElementById("main-content"))})})}function ue(e){const t=e.querySelector(".digest-generate");t&&t.addEventListener("click",()=>{fe(),d("/digest"),ue(document.getElementById("main-content"))});const o=e.querySelector(".digest-copy");o&&o.addEventListener("click",async()=>{const s=b(),n=q(s);try{await navigator.clipboard.writeText(n),o.textContent="Copied",setTimeout(()=>{o.textContent="Copy Digest to Clipboard"},2e3)}catch{o.textContent="Copy failed",setTimeout(()=>{o.textContent="Copy Digest to Clipboard"},2e3)}});const a=e.querySelector("[data-digest-email]");if(a){const s=b(),n=encodeURIComponent(q(s)),r=encodeURIComponent("My 9AM Job Digest");a.href=`mailto:?subject=${r}&body=${n}`}}function ge(e){e.querySelectorAll(".proof-input").forEach(o=>{const a=o.dataset.proofKey;a&&o.addEventListener("blur",()=>{Se(a,o.value),d("/jt/proof"),ge(document.getElementById("main-content"))})});const t=e.querySelector(".proof-copy-submission");t&&t.addEventListener("click",async()=>{const o=Ue();try{await navigator.clipboard.writeText(o),y("Final submission copied to clipboard.")}catch{y("Copy failed.")}})}function P(e){e.querySelectorAll(".test-item__checkbox").forEach(o=>{o.addEventListener("change",a=>{ke(a.target.dataset.testId,a.target.checked),d("/jt/07-test"),P(document.getElementById("main-content"))})});const t=e.querySelector(".test-reset");t&&t.addEventListener("click",()=>{be(),d("/jt/07-test"),P(document.getElementById("main-content"))})}function R(e){e.querySelectorAll(".job-action[data-action='view']").forEach(t=>{t.addEventListener("click",()=>{const o=u.find(a=>a.id===t.dataset.id);o&&pe(o)})}),e.querySelectorAll(".job-action[data-action='unsave']").forEach(t=>{t.addEventListener("click",()=>{ne(t.dataset.id),d("/saved"),R(document.getElementById("main-content"))})}),e.querySelectorAll(".job-card__status-select").forEach(t=>{t.addEventListener("change",o=>{const a=t.dataset.statusFor,s=o.target.value,n=u.find(r=>r.id===a);K(a,s,n),(s==="Applied"||s==="Rejected"||s==="Selected")&&y(`Status updated: ${s}`),d("/saved"),R(document.getElementById("main-content"))})})}function Fe(e,t){let o;return(...a)=>{clearTimeout(o),o=setTimeout(()=>e(...a),t)}}function x(e){document.querySelectorAll(".app-nav__link[data-route]").forEach(t=>{const o=(t.getAttribute("data-route")||"/").replace(/\/$/,"")||"/";t.classList.toggle("active",o===e)})}function me(e){history.pushState({path:e},"",e||"/"),d(e),x(e)}function Ne(e){const t=e.target.closest(".app-nav a[href^='/']");if(!t)return;e.preventDefault();let o=(t.getAttribute("href")||"/").replace(/\/$/,"")||"/";if(o==="/jt/08-ship"&&M()!=="Shipped"){y("Complete all 10 tests and all 3 artifact links to unlock Ship.");return}me(o),Ye()}function Ye(){const e=document.querySelector(".app-nav__toggle"),t=document.querySelector(".app-nav__links");e&&e.setAttribute("aria-expanded","false"),t&&t.classList.remove("is-open")}function Oe(){const e=document.querySelector(".app-nav__toggle"),t=document.querySelector(".app-nav__links"),o=e.getAttribute("aria-expanded")==="true";e.setAttribute("aria-expanded",!o),t.classList.toggle("is-open",!o)}function He(e){const t=e.target.closest("#main-content a[href^='/']");if(!t||t.classList.contains("job-action--apply"))return;e.preventDefault();let o=(t.getAttribute("href")||"/").replace(/\/$/,"")||"/";if(o==="/jt/08-ship"&&M()!=="Shipped"){y("Complete all 10 tests and all 3 artifact links to unlock Ship.");return}me(o)}function Qe(){const e=N();d(e),x(e),document.querySelector(".app-nav")?.addEventListener("click",Ne),document.getElementById("main-content")?.addEventListener("click",He),document.querySelector(".app-nav__toggle")?.addEventListener("click",Oe),window.addEventListener("popstate",t=>{const o=t.state?.path??N();d(o),x(o)})}Qe();
